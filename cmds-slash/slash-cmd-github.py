...
class GitHub(commands.Cog):
    ...
    async def github(self, ctx: disnake.ApplicationCommandInteraction, user: str):
        ...
        embed = disnake.Embed(
            title="GitHub", description=f"{user}'s Profile", color=0x303136
        )
        ...
        embed.add_field(
            name="Website", value=f'https://{githubjson["blog"]}/', inline=False
        )
        embed.add_field(
            name="Twitter", value=f'`@{githubjson["twitter_username"]}`', inline=True
        )
        embed.add_field(
            name="Followers", value=f'**{githubjson["followers"]}**', inline=True
        )
        embed.add_field(
            name="Following", value=f'**{githubjson["following"]}**', inline=True
        )
        ...
        await ctx.response.send_message(
            content=f'<{githubjson["html_url"]}>', embed=embed
        )
