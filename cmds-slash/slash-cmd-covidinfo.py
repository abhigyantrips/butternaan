            ...
            for state_ut in jsonstate:
                if state_ut["loc"] == state:
                    embed.add_field(
                        ...
                        value=state_ut["confirmedCasesIndian"],
                        inline=True,
                    )
                    embed.add_field(
                        name="Confirmed Cases (Foreign)",
                        value=state_ut["confirmedCasesForeign"],
                        inline=True,
                    )
                    embed.add_field(
    ...
    async def _resources(
        ...
        if contactjson["success"] and resourcejson["success"]:
            ...
            for state_ut in contactstate:
                if state_ut["loc"] == state:
                    embed.add_field(
                        name=f"Local Contact Helpline",
                        value=f'{state} - {state_ut["number"]}',
                        inline=False,
                    )
                    embed.add_field(
                        name=f"National Contact Helpine",
                        value=f'```\nNumber: {contactjson["data"]["contacts"]["primary"]["number"]}\nToll-free: {contactjson["data"]["contacts"]["primary"]["number-tollfree"]}\nE-mail: {contactjson["data"]["contacts"]["primary"]["email"]}```',
                        inline=False,
                    )
            ...
            for state_ut in hospbedstate:
                if state_ut["state"] == state:
                    embed.add_field(
                        name=f"Available Hospitals & Beds",
                        value=f'```Total Hospitals: {state_ut["totalHospitals"]}\n(Urban - {state_ut["urbanHospitals"]}; Rural - {state_ut["ruralHospitals"]})\nTotal Beds: {state_ut["totalBeds"]}\n(Urban - {state_ut["urbanBeds"]}; Rural - {state_ut["ruralBeds"]})```',
                        inline=False,
                    )
        else:
