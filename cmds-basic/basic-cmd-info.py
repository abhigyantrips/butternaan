...
class GetInfo(commands.Cog):
    ...
    async def _server(self, ctx):
        ...
        online = len([m.status for m in guild.members
                      if m.status == disnake.Status.online or
                      m.status == disnake.Status.idle])
    ...
    async def _bot(self, ctx):
        ...
        em = disnake.Embed(description='Some current stats for Spectrum', colour=disnake.Colour(value=0x36393e))
