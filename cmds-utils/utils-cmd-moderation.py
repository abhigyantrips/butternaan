...
class Moderation(commands.Cog):
    ...
    @mod.sub_command(
        ...
        options = [
            Option('amount', 'Amount of messages to be cleared.', OptionType.integer, True)
        ]
    )
    ...
    @mod.sub_command(
        ...
        options = [
            Option('user', 'The user to delete messages of.', OptionType.user, True),
            Option('channel', 'Channel to delete messages from.', OptionType.channel, True, None, None, [ChannelType.text])
        ]
    )
    async def channelprune(self, ctx: ApplicationCommandInteraction, user: disnake.Member, channel: disnake.TextChannel):
        ...
        await ctx.edit_original_message(content = f'Deleted **{len(deleted)}** messages by `{user}` in {channel.mention}.')
    ...
    async def ban(self, ctx: ApplicationCommandInteraction, user, reason = None):
        ...
        if reason == None or ' ':
            ...
            await ctx.response.send_message(f'Alrighty, banned `{user}`. \nNo reason given.')
        else:
            await user.send(f'You got banned lel. \nReason: `{reason}`')
            await ctx.response.send_message(f'Alrighty, banned `{user}`. \nReason: `{reason}`')
