...
class EventLogging(commands.Cog):
    ...
    async def on_message_edit(self, before, after):
        ...
        if before.message.guild.id == int(os.environ.get("TEST_GUILDS_ONE")):
            embed = disnake.Embed(
            ...
            await self.client.get_channel(891527413866053653).send(
                embed=embed
            )  # The ID refers to the server log in my server.
    ...
    async def on_message_delete(self, message):
        ...
        if before.message.guild.id == int(os.environ.get("TEST_GUILDS_ONE")):
            ...
            embed = disnake.Embed(
                title="Message Deletion.", color=0x303136, description=desc
            )
            ...
            if message.reference is not None and not message.is_system:
                ...
                embed.add_field(
                    name=f"Replying to {ref_msg.author.mention}",
