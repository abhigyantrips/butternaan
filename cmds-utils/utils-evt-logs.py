...
class Logs(commands.Cog):
    ...
    async def on_message_delete(self, message: disnake.Message):
        ...
        embed.add_field(name="Timestamp", value=f"<t:{round(time.mktime(message.created_at.timetuple()))}:f>", inline=False)

        if message.reference:
            reference = self.client.fetch_message(message.reference.message_id)
            embed.add_field(name="Reference", value=f"{reference.author.mention} - {reference.content}", inline=False)
    ...
    async def on_message_edit(self, before: disnake.Message, after: disnake.Message):
        ...
        if (before.guild.id != int(os.getenv("TEST_GUILDS_ONE"))) or (before.author.bot) or (not before.content) or (before.content == after.content):
            return
        ...
        embed.add_field(name="Timestamp", value=f"<t:{round(time.mktime(after.created_at.timetuple()))}:f>", inline=False)
        embed.add_field(name="Edited", value=f"<t:{round(time.mktime(after.edited_at.timetuple()))}:f>", inline=False)
    ...
    async def on_reaction_clear(self, message: disnake.Message, reactions: typing.List[disnake.Reaction]):
        ...
        if (message.guild.id != int(os.getenv("TEST_GUILDS_ONE"))) or (message.author.bot) or (not message.content) or (not reactions):
            return
        ...
        embed.add_field(name="Content", value=message.content or "None", inline=False)
        embed.add_field(name="Reactions", value=" ".join(str(reaction.emoji) for reaction in reactions), inline=False)
    ...
    async def on_guild_channel_create(self, channel: disnake.abc.GuildChannel):
        ...
        embed.add_field(name="Position", value=f"Category - {channel.category.name}; Position - {channel.position}", inline=False)
        embed.add_field(name="Created", value=f"<t:{round(time.mktime(channel.created_at.timetuple()))}:f>", inline=False)
